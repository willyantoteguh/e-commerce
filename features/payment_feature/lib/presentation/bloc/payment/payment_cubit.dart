...
class PaymentCubit extends Cubit<PaymentState> {
  ...
  Future<void> _onFailureGetPaymentMethod(
    ...
    emit(
      state.copyWith(
        paymentMethodState: ViewData.error(message: 'Error', failure: failure),
      ),
    );
  }
  ...
  Future<void> _onSuccessGetPaymentMethod(
    ...
    if (data.isEmpty) {
      emit(
        state.copyWith(paymentMethodState: ViewData.noData(message: 'No Data')),
      );
    } else {
      emit(
        state.copyWith(paymentMethodState: ViewData.loaded(data: data)),
      );
    }
  }
  ...
  Future<void> _onFailureCreateTransaction(
    ...
    emit(
      state.copyWith(
        createPaymentState: ViewData.error(message: 'Error', failure: failure),
      ),
    );
  }
