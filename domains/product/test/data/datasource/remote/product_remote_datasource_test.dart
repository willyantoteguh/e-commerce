...
void testProductRemoteDataSourceTest() {
  ...
  ''', () async {
    ...
    final httpResponse = ResponseBody.fromString(responseJson, HttpStatus.ok, headers: {
      Headers.contentTypeHeader: [Headers.jsonContentType],
    });

    when(() => dioAdapterMock.fetch(any(), any(), any())).thenAnswer((invocation) async => httpResponse);
  ...
  ''', () async {
    //GIVEN
    when(() => dioAdapterMock.fetch(any(), any(), any())).thenThrow(const FailureResponse(errorMessage: 'Internal Server Error'));
